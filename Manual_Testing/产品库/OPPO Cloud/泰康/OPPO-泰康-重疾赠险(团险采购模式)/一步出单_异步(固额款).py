...
回调关键词查询:TK_GROUP_INSURE_CALLBACK_QUEUE,泰康团险出单回调原始报文
kibana 通过关键字:"回调通知" ,查看回调
记录:多次承保场景(实际不存在该场景);定时任务,上一次核保-承保-出单更新数据库完成后,再执行承保任务不会报错,否则多次承保第二次会报错个人保险器件重叠
流程注: 一步出单请求成功>泰康团险重疾B款核保任务>泰康团险重疾B款核保承保任务(等待回调成功)>异步出单更新数据库任务>异步出单推送销售方任务
"""
        ...
        body = {
            "Data": {
                "Policy": {
                    ...
                    "FaceAmount": "20000.00",  # 保额,注：1、保额可批增为初始保额2、医疗险多种条款为保额之和,(1000,5000,10000,20000)
                ...
                "InsuredList": [
                    {
                        ...
                        "IdNumber": CO.newIdNum(1973, 1, 7),  # 证件号
                        "Mobile": None,  # 手机号
                        "BirthDate": CO.Birthday(CO.newIdNum(1973, 1, 7)),  # 出生日期
                        "Email": CO.Mailbox(),  # 邮箱
